...
switch (playerSide) do
{
	case civilian:
	{
		...
		//Destroy Radios
		life_actions = life_actions + [player addAction[localize "STR_pAct_DestroyRadio",life_fnc_civremoveradio,"",0,false,false,"",' player distance cursorTarget < 3.5 && isPlayer cursorTarget && animationState cursorTarget == "Incapacitated" && !(cursorTarget getVariable["radiodestroyed",FALSE]) ']];
	};
	case west:
	{
	// Heli Fast Rope
